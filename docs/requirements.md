...
| E1-06 | — | Een oefening heeft een optionele lateraliteit: bilateraal (beide ledematen tegelijk) of unilateraal (één per keer); onbekend is toegestaan. De keuze is instelbaar op het oefening-formulier, zowel bij het aanmaken van een custom oefening als bij het bewerken. De 25 standaard-oefeningen krijgen een correcte lateraliteit via seed en een backfill-migratie (bestaande installaties). |
| E1-07 | — | Het oefening-formulier biedt een veld "Standaard rust" (per-oefening rusttijd tussen sets) met +/− stepper in stappen van 15 seconden, minimum 15s en maximum 600s. Een leeg veld betekent dat de lateraliteit-default en anders de globale rustinstelling gelden; het veld toont die geërfde waarde als placeholder. Een reset-actie zet de waarde terug naar leeg. |
...
| E3-15 | — | De rusttijdsduur is per oefening overschrijfbaar: per oefening in een schema (E2-12), als per-oefening standaard (E1-07) en vanuit het live loggingscherm via een contextmenu per oefening. De effectieve rusttijd wordt bepaald van specifiek naar algemeen: (1) rust bij de oefening in het schema, (2) per-oefening standaardrust, (3) lateraliteit-default (bilateraal = globaal + offset, zie E8-09), (4) globale instelling. De eerste gezette waarde wint. |
...
| E8-09 | — | De gebruiker kan de extra rusttijd voor bilaterale oefeningen instellen (`bilateralRestExtraSeconds`, standaard 60s) in stappen van 15 seconden, minimum 0s en maximum 600s. Deze offset wordt bij de globale rustinstelling opgeteld (gecapt op 600s) als afgeleide standaardrust voor bilaterale oefeningen zonder eigen rustwaarde (zie E3-15). |
