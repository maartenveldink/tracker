...
| E1-06 | ✓ | Een oefening heeft een optionele lateraliteit: bilateraal (beide ledematen tegelijk) of unilateraal (één per keer); onbekend is toegestaan. Instelbaar op het oefening-formulier (tap-to-cycle toggle), zowel bij aanmaken van een custom oefening als bij bewerken. De 25 standaard-oefeningen krijgen een correcte lateraliteit via seed en een backfill-migratie. |
| E1-07 | ✓ | Het oefening-formulier biedt een veld "Standaard rust" (per-oefening rusttijd tussen sets) met +/− stepper in stappen van 15 seconden, minimum 15s en maximum 600s. Een leeg veld betekent dat de type-default (E8-09) en anders de globale rustinstelling gelden; het veld toont die geërfde waarde als placeholder. Een reset-actie zet de waarde terug naar leeg. |
| E1-08 | ✓ | Een oefening heeft een optioneel bewegingstype: compound (meerdere gewrichten) of isolatie; onbekend is toegestaan. Instelbaar op het oefening-formulier (tap-to-cycle toggle), naast de lateraliteit. De 25 standaard-oefeningen krijgen een correct bewegingstype via seed en een backfill-migratie. |
...
| E2-12 | ✓ | In het schema-formulier heeft elke oefening een rust-veld (rusttijd tussen sets) met een +/− stepper in stappen van 15 seconden, weergegeven als `M:SS` (bv. `1:30`), naast de bestaande velden voor sets en reps. Minimum 15s, maximum 600s. Bij het toevoegen is er geen harde standaardwaarde: het veld blijft leeg en toont de geërfde waarde (per-oefening of globaal) als placeholder. De waarde wordt per oefening in het schema opgeslagen en blijft behouden na heropenen. Een reset-actie zet de waarde terug naar leeg. |
...
| E3-15 | ✓ | De rusttijdsduur is per oefening overschrijfbaar: per oefening in een schema (E2-12), als per-oefening standaard (E1-07) en vanuit het live loggingscherm per oefening. De effectieve rusttijd wordt bepaald van specifiek naar algemeen: (1) rust bij de oefening in het schema, (2) per-oefening standaardrust, (3) type-default uit de matrix lateraliteit × bewegingstype (zie E8-09), (4) globale instelling. De type-default geldt alleen als lateraliteit én bewegingstype bekend zijn; anders geldt de globale instelling. De eerste gezette waarde wint. |
...
| E8-09 | ✓ | De gebruiker kan de standaardrust per type oefening instellen via een 2×2-matrix (`AppSettings.restDefaults`): bilateraal-compound (standaard 3:00), unilateraal-compound (1:30), bilateraal-isolatie (1:00) en unilateraal-isolatie (0:15). Elke waarde is instelbaar in stappen van 15 seconden, minimum 15s en maximum 600s. Deze waarden vormen de afgeleide standaardrust voor oefeningen met bekende lateraliteit én bewegingstype zonder eigen rustwaarde (zie E3-15). Vervangt de eerdere `bilateralRestExtraSeconds`-offset. |
